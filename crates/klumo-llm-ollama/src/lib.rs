...
use klumo_llm::{LlmClient, LlmTranslateRequest};
...
mod tests {
    ...
    fn live_ollama_translate_if_enabled() {
        if std::env::var("KLUMO_RUN_LIVE_TESTS").ok().as_deref() != Some("1") {
            return;
        }

        let base = std::env::var("KLUMO_OLLAMA_URL")
            .unwrap_or_else(|_| "http://127.0.0.1:11434".to_string());
        let model =
            std::env::var("KLUMO_OLLAMA_MODEL").unwrap_or_else(|_| "qwen2.5-coder:7b".to_string());
