...
RUN_OUT="$(cargo run -p beeno -- run "$ROOT_DIR/tests/fixtures/math.js")"
if [[ "$RUN_OUT" != *"42"* ]]; then
...
BUNDLE_OUT="$(cargo run -p beeno -- bundle "$ROOT_DIR/tests/fixtures/math.js" --output "$TMP_DIR/math.bundle.js")"
...
BUNDLED_RUN_OUT="$(cargo run -p beeno -- run "$TMP_DIR/math.bundle.js")"
